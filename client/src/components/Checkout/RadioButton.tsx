          ...
          {({ active, checked }) => (
            <div className="flex items-center space-x-4 w-full p-5">
              <span
                ...
                id={name}
              ></span>
              <RadioGroup.Label as="label" className="flex text-base font-bold text-gray-900" htmlFor={name}>
                {title}
                {icon && (
                  <div className="w-[200px]">
                    <img src={icon} alt="image" className="w-[175px] sm:w-full h-full object-contain border ml-4" />
                  </div>
                )}
              </RadioGroup.Label>
            </div>
